...
def create_category(
    ...
    if not cat_data.is_global and cat_data.user_id:
        if cat_data.user_id != current_user.id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="Cannot create private category for another user")
...
def get_categories(
    ...
    if user_id and user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Cannot view private categories of another user")
...
def delete_category(
    ...
    if not category.is_global and category.user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Not authorized to delete this private category")
